...
export function participantSharesFromExisting(
  ...
  if (!previousShares?.length) return undefined;
...
export function previewParticipantShares(
  mode: SplitMode,
  participantIds: string[],
  amountMinor: number,
  ...
  const persisted = participantSharesFromBody(
    { splitMode: mode, splitValues: values },
    participantIds,
    amountMinor,
    currency,
  );
...
export function participantSharesFromBody(
  ...
  if (mode === "amount") {
    ...
    if (
      shares.reduce((sum, share) => sum + share.shareMinor, 0) !== amountMinor
    ) {
