...
import { withCredentialLock } from "../src/auth.js";
  ...
  test.each([
    {
      ...
      message: "USD supports 2 decimal places",
    },
    {
      args: ["trips", "create", "--name", "Trip", "--currency", "BTC"],
    ...
    "rejects invalid local input before request construction",
    ({ args, message }) => {
      expect(() => parseCliCommand(args)).toThrow(message);
    },
  );
