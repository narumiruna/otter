...
export function parseCliCommand(args: string[]): CliCommand {
  ...
  switch (`${resource}:${action}`) {
    ...
    case "trips:create": {
      ...
      return {
        body: compact({
          baseCurrency: optionalCurrency(options, "currency"),
...
function expenseAddCommand(options: ParsedOptions): CliCommand {
  ...
  const tripId = required(options, "trip");
  const currency = requiredCurrency(options, "currency");
  const amount = required(options, "amount");
  validateAmount(amount, currency);
  return {
    body: compact({
      amount,
      category: optionalCategory(options, "category"),
      currency,
      description: required(options, "description"),
      expenseDate: optionalDate(options, "date"),
...
function expenseUpdateCommand(options: ParsedOptions): CliCommand {
  ...
  const tripId = required(options, "trip");
  const expenseId = required(options, "expense");
  const participantIds = optionalNonEmptyList(options, "split-with");
  const currency = optionalCurrency(options, "currency");
  const amount = optional(options, "amount");
  if (amount !== undefined) {
    validateAmount(amount, currency);
  }
  const body = compact({
...
function settlementRecordCommand(options: ParsedOptions): CliCommand {
  ...
  return {
    body: compact({
      ...
      paidAt: optionalDate(options, "date"),
...
function requiredCurrency(options: ParsedOptions, name: string): Currency {
  const value = required(options, name);
  if (!isCurrency(value)) {
    throw new CliError("USAGE", `Option --${name} has an unsupported currency`);
  }
  return value;
}
...
function optionalDate(
  ...
  if (value !== undefined && !isDateOnly(value)) {
    throw new CliError("USAGE", `Option --${name} must use YYYY-MM-DD`);
  }
  return value;
}
...
function optionalCategory(
  ...
  const value = optional(options, name);
  if (value !== undefined && !isExpenseCategory(value)) {
    throw new CliError("USAGE", `Option --${name} has an unsupported category`);
  }
  return value;
}

function validateAmount(amount: string, currency?: Currency): void {
  if (currency) {
    try {
  ...
  if (
    ...
    throw new CliError("USAGE", "Amount must be a positive number");
  }
}
...
function optionalNonEmptyList(
  options: ParsedOptions,
  name: string,
): string[] | undefined {
  const value = optionalList(options, name);
